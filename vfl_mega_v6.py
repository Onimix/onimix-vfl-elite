#!/usr/bin/env python3
"""
ONIMIX VFL MEGA Accumulator v6 — UPGRADED LOGIC
Key upgrades from v5:
1. ULTRA MEGA: Only DIAMOND tier (100% matchups), max 5-fold
2. MEGA MIX: DIAMOND + GOLD (≥86%), max 4-fold
3. SAFE BET: Best 3 DIAMOND picks only (highest BTTS for extra confidence)
4. Danger team penalty from rolling failures
5. No more FULL HOUSE (too many legs = guaranteed loss)
6. Minimum games threshold raised to 7
"""
import requests, json, hashlib, time, os, random
from datetime import datetime, timezone, timedelta

# ─── CONFIG ───
TG_TOKEN = "8616919960:AAFY5dY8-MyOgahSKpVeDKD_ESPZVVJ-tb8"
TG_CHAT  = "1745848158"
DEDUP_FILE = "/tmp/vfl_dedup_MEGA_v6.json"
ALERT_WINDOW = 20
...
def get_booking_code(selections):
    ...
    try:
        r = requests.post(url, json={"selections": selections}, headers=HEADERS, timeout=10)
...
def fetch_upcoming():
    ...
    for cat_enc, tourn_enc, lg in cats:
        try:
            url = f"{API_BASE}/factsCenter/eventResultList?pageNum=1&pageSize=100&sportId=sr%3Asport%3A202120001&categoryId={cat_enc}&tournamentId={tourn_enc}&startTime={now_ts}&endTime={now_ts + ALERT_WINDOW*60*1000}"
...
def build_selections(picks):
    return [{"eventId": p["eventId"], "marketId": "total", "specifier": "total=1.5", "outcomeId": "over"} for p in picks]

# ─── MAIN ───
def main():
    print("ONIMIX VFL MEGA Accumulator v6")
    print(f"ELITE DB: {len(ELITE_DB)} matchups")
    ...
    for e in events:
        key = f"{e['home']}v{e['away']}"
        if key in ELITE_DB:
            info = ELITE_DB[key]
            pick = {**e, "tier": info["t"], "rate": info["r"], "games": info["g"], "btts": info["b"]}
            if info["t"] == "D":
                diamond_picks.append(pick)
            elif info["t"] == "G":
                gold_picks.append(pick)
    
    print(f"Diamond: {len(diamond_picks)}, Gold: {len(gold_picks)}")
    
    if not diamond_picks and not gold_picks:
        print("No ELITE picks. Silent exit.")
        return
    
    # Dedup on all available picks
    all_ids = [p["eventId"] for p in diamond_picks + gold_picks]
    h = make_hash(all_ids)
    dedup = load_dedup()
    if h in dedup:
    ...
    msg = f"🏆 <b>ONIMIX MEGA v6</b> 🏆\n"
    msg += f"📅 {now_wat.strftime('%H:%M WAT %d/%m/%Y')}\n"
    msg += f"🎯 Market: Over 1.5 Goals\n\n"
    
    tiers_sent = 0
    
    # TIER 1: SAFE BET — Best 3 DIAMOND picks (sorted by BTTS for extra confidence)
    if len(diamond_picks) >= 3:
        safe_picks = sorted(diamond_picks, key=lambda x: x["btts"], reverse=True)[:3]
        sels = build_selections(safe_picks)
        code = get_booking_code(sels)
        
        msg += f"🔒 <b>SAFE BET</b> (3-fold DIAMOND)\n"
        msg += f"📊 Expected: ~100% per leg\n"
        for p in safe_picks:
            msg += f"  💎 {p['home']} vs {p['away']} ({p['league']})\n"
            msg += f"     {p['games']}g=100% | BTTS {p['btts']*100:.0f}%\n"
        msg += f"📋 Code: <code>{code}</code>\n\n"
        tiers_sent += 1
    
    # TIER 2: ULTRA MEGA — Up to 5 DIAMOND picks
    if len(diamond_picks) >= 4:
        ultra_picks = sorted(diamond_picks, key=lambda x: (x["games"], x["btts"]), reverse=True)[:5]
        sels = build_selections(ultra_picks)
        code = get_booking_code(sels)
        
        msg += f"💎 <b>ULTRA MEGA</b> ({len(ultra_picks)}-fold DIAMOND)\n"
        msg += f"📊 Expected: 100% per leg\n"
        for p in ultra_picks:
            msg += f"  💎 {p['home']} vs {p['away']} ({p['league']})\n"
            msg += f"     {p['games']}g=100% | BTTS {p['btts']*100:.0f}%\n"
        msg += f"📋 Code: <code>{code}</code>\n\n"
        tiers_sent += 1
    
    # TIER 3: MEGA MIX — Best 4 from DIAMOND+GOLD combined (prioritize DIAMOND)
    if len(diamond_picks) + len(gold_picks) >= 4:
        # Take all diamonds first, then top golds
        combined = sorted(diamond_picks, key=lambda x: x["btts"], reverse=True) + \
                   sorted(gold_picks, key=lambda x: (x["rate"], x["btts"]), reverse=True)
        mega_picks = combined[:4]
        
        # Only if we have at least 1 gold (otherwise it's same as ultra)
        has_gold = any(p["tier"] == "G" for p in mega_picks)
        if has_gold:
            sels = build_selections(mega_picks)
            code = get_booking_code(sels)
            
            msg += f"🥇 <b>MEGA MIX</b> (4-fold D+G)\n"
            msg += f"📊 Expected: ≥86% per leg\n"
            for p in mega_picks:
                icon = "💎" if p["tier"] == "D" else "🥇"
                msg += f"  {icon} {p['home']} vs {p['away']} ({p['league']})\n"
                msg += f"     {p['games']}g={p['rate']*100:.0f}% | BTTS {p['btts']*100:.0f}%\n"
            msg += f"📋 Code: <code>{code}</code>\n\n"
            tiers_sent += 1
    
    if tiers_sent == 0:
        # Not enough picks for any tier
        if len(diamond_picks) >= 2:
            safe_picks = sorted(diamond_picks, key=lambda x: x["btts"], reverse=True)[:2]
            sels = build_selections(safe_picks)
            code = get_booking_code(sels)
            msg += f"🔒 <b>SAFE DOUBLE</b> (2-fold DIAMOND)\n"
            for p in safe_picks:
                msg += f"  💎 {p['home']} vs {p['away']} ({p['league']})\n"
            msg += f"📋 Code: <code>{code}</code>\n\n"
            tiers_sent += 1
        else:
            print("Not enough picks for any mega tier. Silent exit.")
            return
    
    msg += f"💎 Diamond: {len(diamond_picks)} | 🥇 Gold: {len(gold_picks)}\n"
    msg += f"🔒 <i>MEGA v6 — Max 5-fold, DIAMOND-first strategy</i>"
    
    send_telegram(msg)
    print(f"✅ Sent {tiers_sent} MEGA tiers to Telegram")
