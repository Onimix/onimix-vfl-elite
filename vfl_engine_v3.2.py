"""
VFL Prediction Engine v3.3 — ONIMIX TECH
...
+ Hard 1.38 minimum odds filter (below = trap)
+ matchStatus / bookingStatus / isActive checks
...
def event_detail(eid):
    ...
    if d and d.get('bizCode') == 10000 and d.get('data', {}).get('homeTeamName'):
        ev = d['data']
        # Audit fix: skip if matchStatus != 0 (not pre-match)
        if ev.get('matchStatus') not in (None, 0, '0'):
            return None
        # Audit fix: skip if bookingStatus is closed
        bs = ev.get('bookingStatus')
        if bs is not None and bs not in (0, '0', 'Booked', 'booked'):
            return None
        return ev
    return None
...
def sec_a(event):
    ...
    for m in mkts:
        ...
        for o in m.get('outcomes', []):
            try:
                # Audit fix: skip inactive outcomes (isActive != 1)
...
def analyze(event, yest):
    ...
    # ── HARD MINIMUM 1.38 — anything below is a trap ──
