#!/usr/bin/env python3
"""ONIMIX VFL Mega Prematch Scanner v4 + FEEDBACK + DEDUP"""
import urllib.request, json, ssl, time, datetime, os, hashlib
...
def make_picks_hash(picks):
    keys=sorted([p.get('eid','') for p in picks])
...
def filter_already_sent_eids(picks,dd):
    ...
    dd['sent_eids']=sent
    return [p for p in picks if p.get('eid','') not in sent]
        ...
        for rnd in range(1, rounds + 1):
            ...
            for oc in outcomes:
                ...
                if rate >= MIN_MEGA:
                    ...
                    if cold and rate < MIN_PREMIUM: continue
                    all_picks.append({
...
def scan_live_elite(league_info):
    ...
    for lg, info in league_info.items():
        for e in info.get('events',[]):
            ...
            # STRICT: Only include very early matches (<=5 min)
            if mins > 5: continue
            home = e.get('homeTeamName',''); away = e.get('awayTeamName','')
            mu = f"{home[:3].upper()} vs {away[:3].upper()}"
            rate = 0
...
def build_msg(tier_name, emoji, picks, code, share_url, combined):
    ...
    lines = [
        f"{emoji} ONIMIX VFL {tier_name} ACCA v4",
    ...
    lines.append("")
    lines.append("🤖 ONIMIX AI v4 | Dedup+Feedback | 73K DB")
    return '\n'.join(lines)
...
def main():
    print("🚀 ONIMIX Mega v4 DEDUP starting...")
    dd = load_dedup()
    
    league_info = get_live_events()
    if not league_info:
        print("❌ No live events - silent exit")
        return
    ...
    print(f"✅ Live ELITE (<=5min): {len(live_elite)}")
    ...
    # DEDUP: Filter already-sent event IDs
    unique = filter_already_sent_eids(unique, dd)
    print(f"✅ After dedup filter: {len(unique)}")
    
    if not unique:
        # NO "no picks" message - silent exit to save credits
        print("No new ELITE picks - silent exit (no TG spam)")
        return

    # DEDUP CHECK: Same overall set?
    if is_duplicate(unique, dd):
    ...
    if all_sent:
        mark_sent(all_sent, dd)
        log_picks(all_sent, 'MEGA')
    print("\n🏁 Mega v4 DEDUP complete!")
