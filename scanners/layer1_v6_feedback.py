    ...
    for attempt in range(retries):
        try:
            ...
            if data:
                ...
                h['Referer']='https://www.sportybet.com/ng/virtual'
                h['Origin']='https://www.sportybet.com'
...
try:
    if os.path.exists(FEEDBACK_FILE):
        with open(FEEDBACK_FILE) as _ff:
            _feedback = json.load(_ff)
        print('Feedback: '+str(len(_feedback.get('bl',{})))+' blacklisted, '+str(len(_feedback.get('pen',{})))+' penalized, cold:'+str(_feedback.get('cold',[])))
    else:
        print('No feedback file - first run')
except:
    print('Feedback load failed')

def is_blacklisted(mu, hour):
    bl = _feedback.get('bl', {})
    if mu in bl and abs(bl[mu] - hour) <= 1:
        return True
    return False

def get_penalty(mu, hour):
    pen = _feedback.get('pen', {})
    if mu in pen and abs(pen[mu] - hour) <= 1:
        return 3
    return 0
...
def probe_prematch_elite(league_info, elite_lookup, rounds=5):
    prematch_picks = []
    cur_hour = datetime.now(WAT).hour
    for lg, info in league_info.items():
        for rnd in range(1, rounds + 1):
            start_eid = info['max_eid'] + 1 + (rnd - 1) * info['count']
            selections = [{'eventId': 'sr:match:' + str(start_eid + i),'marketId': 18, 'specifier': 'total=1.5', 'outcomeId': '12'} for i in range(info['count'])]
            payload = json.dumps({"selections": selections, "sportId": SPORT}).encode()
            resp = fetch_json(BOOK_URL, data=payload)
            if not resp: break
            outcomes = resp.get('data', {}).get('outcomes', [])
            if not outcomes: break
            for oc in outcomes:
                home = oc.get('homeTeamName', '')
                away = oc.get('awayTeamName', '')
                eid = oc.get('eventId', '')
                ko = oc.get('estimateStartTime', 0)
                h3 = home[:3].upper()
                a3 = away[:3].upper()
                mu = h3 + ' vs ' + a3
                o15_odds = None
                for mkt in oc.get('markets', []):
                    if str(mkt.get('id')) == '18':
                        for out in mkt.get('outcomes', []):
                            if str(out.get('id')) == '12':
                                o15_odds = out.get('odds')
                if mu in elite_lookup:
                    _, rate = elite_lookup[mu]
                    # FEEDBACK: Skip blacklisted matchups
                    if is_blacklisted(mu, cur_hour):
                        print('  🚫 SKIP (blacklisted): '+mu)
                        continue
                    # FEEDBACK: Note if cold hour
                    cold_tag = ' ❄️' if is_cold_hour(cur_hour) else ''
                    prematch_picks.append({'mu': mu, 'league': lg, 'rate': rate,
                        'eventId': eid, 'odds': str(o15_odds) if o15_odds else '1.27',
                        'ko_est': ko, 'round': rnd, 'home': home, 'away': away,
                        'cold': is_cold_hour(cur_hour)})
            time.sleep(0.3)
    prematch_picks.sort(key=lambda x: (-x['rate'], x['ko_est']))
    return prematch_picks
...
cur_hour=datetime.now(WAT).hour
print('Layer 1 v6 FEEDBACK | AW:'+str(AW)+'min | '+now_s+' WAT')
...
try:
    ...
    print('Scanned '+str(total_events)+' live events')
    ...
    for league,ld in league_data.items():
        ...
        for mu in ld['matchups']:
            if mu in elite_lookup:
                ...
                # FEEDBACK CHECK
                if is_blacklisted(mu, cur_hour):
                    print('  🚫 BLACKLISTED: '+mu+' (skipping)')
                    skipped_bl+=1
                    continue
                if is_cold_hour(cur_hour) and rate < 88:
                    print('  ❄️ COLD HOUR skip: '+mu+' (rate '+str(rate)+'% < 88%)')
                    skipped_cold+=1
                    continue
    ...
    if skipped_bl or skipped_cold:
        print('Feedback filter: '+str(skipped_bl)+' blacklisted, '+str(skipped_cold)+' cold-hour skipped')
    print('Found '+str(len(alerts))+' ELITE picks in '+str(AW)+'min window')

    prematch_picks = []
    if league_info:
        prematch_picks = probe_prematch_elite(league_info, elite_lookup, rounds=5)
        print('Found '+str(len(prematch_picks))+' prematch ELITE picks')
    ...
    if alerts:
        ...
        lines=['<b>⚽ VFL ELITE ALERT v6</b>','🕐 '+now_s+' WAT','']
        if skipped_bl or skipped_cold:
            lines.append('🧠 Feedback: '+str(skipped_bl)+' blacklisted, '+str(skipped_cold)+' cold-skipped')
            lines.append('')
        ...
        lines.append('')
        lines.append('🧠 Feedback-enhanced | v6')
        msg='\n'.join(lines)
        if len(msg)>4000:
            top=alerts[:10]
            lines=['<b>⚽ VFL ELITE (Top 10)</b> '+now_s+' WAT','']
            for a in top:
                lines.append(a['flag']+' '+a['mu']+' | '+str(a['rate'])+'% @'+str(a.get('odds','')))
            if book_code:
                lines.append('🎫 <code>'+book_code+'</code>')
                lines.append('<a href="'+book_url+'">👉 BETSLIP</a>')
            msg='\n'.join(lines)
        if send_tg(msg): print('SENT '+str(len(alerts))+' alerts')
        log_picks(alerts, 'L1')
    elif prematch_picks:
        print('No live window - delivering prematch')
        top_picks=prematch_picks[:20]
        pm_sels=[{'eventId':p['eventId'],'odds':p['odds']} for p in top_picks]
        book_code,book_url=create_booking_code(pm_sels)
        combo_odds=1.0
        for p in top_picks:
            try: combo_odds*=float(p['odds'])
            except: pass
        by_league={}
        for p in top_picks:
            lg=p['league']
            if lg not in by_league: by_league[lg]=[]
            by_league[lg].append(p)
        lines=['<b>⚽ VFL ELITE PREMATCH v6</b>','🕐 '+now_s+' WAT','']
        for lg in sorted(by_league.keys()):
            picks=by_league[lg]
            ...
            for p in picks:
                ko_str=''
                if p.get('ko_est'):
                    ko_str=' ⏰'+datetime.fromtimestamp(p['ko_est']/1000,tz=WAT).strftime('%H:%M')
                cold_str=' ❄️' if p.get('cold') else ''
                lines.append('  '+p['mu']+' | <b>'+str(p['rate'])+'%</b> @'+p['odds']+ko_str+cold_str)
            lines.append('')
        lines.append('✅ '+str(len(top_picks))+' ELITE prematch')
        if combo_odds>1: lines.append('💰 Odds: <b>'+str(round(combo_odds,2))+'</b>')
        if book_code and book_url:
            lines.append('')
            lines.append('🎫 <b>ONE-TAP BET:</b>')
            lines.append('Code: <code>'+book_code+'</code>')
            lines.append('<a href="'+book_url+'">👉 TAP TO LOAD BETSLIP</a>')
        lines.append(''); lines.append('🧠 Feedback-enhanced | v6')
        msg='\n'.join(lines)
        if len(msg)>4000:
            top5=top_picks[:8]
            lines=['<b>⚽ ELITE PRE (Top 8)</b> '+now_s+' WAT','']
            for p in top5:
                lines.append(FLAG.get(p['league'],'')+' '+p['mu']+' | '+str(p['rate'])+'% @'+p['odds'])
            if book_code:
                lines.append('🎫 <code>'+book_code+'</code>')
                lines.append('<a href="'+book_url+'">👉 BETSLIP</a>')
            msg='\n'.join(lines)
        if send_tg(msg): print('SENT '+str(len(top_picks))+' prematch')
        log_picks(top_picks, 'L1')
    else:
        print('No ELITE matchups found')
except Exception as ex:
    print('Error: '+str(ex))
    import traceback; traceback.print_exc()

print('DONE - Layer 1 v6 FEEDBACK')
