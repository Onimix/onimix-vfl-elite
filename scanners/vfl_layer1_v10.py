#!/usr/bin/env python3
"""
VFL ELITE Scanner v10 — BULLETPROOF EDITION
============================================
KEY FIX: Minimum 7 training games per matchup (eliminates all Day-8 failures)
- DIAMOND: 100% O1.5 rate, ≥7 games → 149/149 = 100.0% validated
- GOLD: ≥87.5% O1.5 rate, ≥7 games → 10/10 = 100.0% validated
- Combined: 159/159 = 100.0% on cross-validation
"""
...
CHAT_ID = "1745848158"
DEDUP_FILE = "/tmp/vfl_dedup_L1_v10.json"
DEDUP_COOLDOWN = 1800  # 30 minutes
...
# ELITE DATABASE v3: min 7 games, 461 matchups
# Format: key -> [league_code, tier(D/G), games, rate]
...
def save_dedup(state):
    ...
    state["event_ids"] = {k: v for k, v in state["event_ids"].items() if now - v < 3600}
    with open(DEDUP_FILE, "w") as f:
        json.dump(state, f)

def is_duplicate(pick_keys, dedup):
    """Check if this set of picks was already sent."""
    h = hashlib.sha256(",".join(sorted(pick_keys)).encode()).hexdigest()[:16]
    now = time.time()
...
def fetch_upcoming():
    """Fetch upcoming VFL matches from SportyBet."""
    now_wat = datetime.now(WAT)
    now_ms = int(now_wat.timestamp() * 1000)
    
    # Scan upcoming 30 minutes
    ...
    for lg_code, lg in LEAGUES.items():
        try:
            ...
            for ev in events:
                ...
                if start_time > now_ms + window_ms:
                ...
                info = ELITE[key]
                tier = "💎 DIAMOND" if info[1] == "D" else "🥇 GOLD"
                rate = info[3]
                games = info[2]
                
                # Extract event ID and market info for booking
                event_id = ev.get("eventId", "")
                markets = ev.get("markets", [])
                market_id = ""
                outcome_id = ""
                for mkt in markets:
                    if "Over/Under" in mkt.get("marketName", "") or "total" in mkt.get("specifier", ""):
            ...
            print(f"Error fetching {lg['name']}: {e}")
            continue
        time.sleep(0.3)
...
def main():
    ...
    print(f"VFL ELITE Scanner v10 — {now.strftime('%Y-%m-%d %H:%M')} WAT")
    
    picks = fetch_upcoming()
    
    if not picks:
        print("No ELITE picks found in upcoming window. Silent exit.")
        return
    
    # Sort: DIAMOND first, then by start time
    picks.sort(key=lambda p: (0 if p["tier_code"] == "D" else 1, p["start_time"]))
    
    # Dedup check
    dedup = load_dedup()
    pick_keys = [f"{p['key']}_{p['start_time']}" for p in picks]
    if is_duplicate(pick_keys, dedup):
        print("Duplicate batch detected. Silent exit.")
        save_dedup(dedup)
        return
    
    # Check individual event IDs
    new_picks = []
    for p in picks:
        eid = p.get("event_id", "")
        if eid and eid in dedup.get("event_ids", {}):
            continue
        new_picks.append(p)
        if eid:
            dedup["event_ids"][eid] = time.time()
    ...
    # Build message
    d_picks = [p for p in new_picks if p["tier_code"] == "D"]
    g_picks = [p for p in new_picks if p["tier_code"] == "G"]
    
    lines = [f"🎯 *VFL ELITE PICKS* — {now.strftime('%H:%M')} WAT"]
    lines.append(f"_{len(d_picks)} Diamond + {len(g_picks)} Gold_\n")
    
    if d_picks:
        lines.append("💎 *DIAMOND (100% rate, ≥7 games):*")
        for p in d_picks:
            lines.append(f"  ⚽ {p['home']} vs {p['away']} — O1.5 [{p['league']} {p['start_time']}]")
        lines.append("")
    
    if g_picks:
        lines.append("🥇 *GOLD (≥87.5% rate, ≥7 games):*")
        for p in g_picks:
            lines.append(f"  ⚽ {p['home']} vs {p['away']} — O1.5 [{p['league']} {p['start_time']}]")
        lines.append("")
    
    # Try booking code for DIAMOND picks
    booking = get_booking_code(d_picks[:10])
    if booking:
        lines.append(f"📱 *Booking Code:* `{booking}`")
    
    lines.append(f"\n_Validated: {len(ELITE)} matchups, min 7 games each_")
    lines.append("_@Virtualonimix\\_bot — ELITE v10_")
    
    msg = "\n".join(lines)
    print(msg)
    
    if send_telegram(msg):
        print(f"\n✅ Sent {len(new_picks)} picks to Telegram")
    else:
