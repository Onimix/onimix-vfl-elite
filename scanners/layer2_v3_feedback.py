"""VFL LAYER 2 v3 - ONIMIX 12-Layer Engine + FEEDBACK LOOP"""
...
def score_12layer(key, hist_data, live_score=None):
    ...
    # FEEDBACK: Blacklisted = instant skip
    if is_bl(key, cur_hr):
        return 0,'BLACKLISTED',0,0,0
    ...
    # FEEDBACK: Apply penalty from feedback loop
    penalty=get_pen(key, cur_hr)
    if penalty>0:
        score-=penalty
    
    # FEEDBACK: Cold hour = require higher score
...
print('='*50)
print('  LAYER 2 v3 - 12-Layer + FEEDBACK')
print('='*50)
...
try:
    ...
    print('  '+str(total_events)+' events | '+str(len(league_data))+' leagues')
    ...
    for league,ld in league_data.items():
        ...
        for key in ld['matchups']:
            ...
            if cat=='BLACKLISTED':
                print('  🚫 '+key+' BLACKLISTED'); skipped+=1; continue
            if cat=='COLD_SKIP':
                print('  ❄️ '+key+' cold-hour skip (score='+str(sc)+')'); skipped+=1; continue
    ...
    if league_info:
        ...
        print('  Found '+str(len(prematch_picks))+' prematch')
    ...
    if picks:
        ...
        lines=['<b>🧠 LAYER 2 v3 - 12-Layer+FEEDBACK</b>','🕒 '+now_s+' WAT | Over 1.5','']
        ...
        lines.append(''); lines.append('🧠 12-Layer+Feedback | v3')
        msg='\n'.join(lines)
        if len(msg)>4000:
            top=picks[:8]
            ...
            msg='\n'.join(lines)
        if send_tg(msg): print('  SENT '+str(len(picks))+' picks')
        log_picks(picks,'L2')
    elif prematch_picks:
        ...
        for p in top_pm:
            if p['league'] not in by_lg: by_lg[p['league']]=[]
            by_lg[p['league']].append(p)
        lines=['<b>🧠 L2 PREMATCH v3</b>','🕒 '+now_s+' WAT','']
        for lg in sorted(by_lg.keys()):
            lp=by_lg[lg]
        ...
        if book_code and book_url:
            ...
            lines.append('Code: <code>'+book_code+'</code>')
            lines.append('<a href="'+book_url+'">👉 TAP TO LOAD BETSLIP</a>')
        lines.append(''); lines.append('🧠 12-Layer+Feedback Prematch')
        msg='\n'.join(lines)
        if len(msg)>4000:
            ...
            msg='\n'.join(lines)
        if send_tg(msg): print('  SENT '+str(len(top_pm))+' prematch')
        log_picks(top_pm,'L2')
    else:
        send_tg('🧠 <b>L2 v3</b> '+now_s+' WAT\nNo qualified picks. Next scan in 5 min.')

except Exception as ex:
    print('ERROR: '+str(ex))
    import traceback; traceback.print_exc()

print('\nLAYER 2 v3 FEEDBACK DONE')
