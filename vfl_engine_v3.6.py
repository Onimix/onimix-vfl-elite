...
def load_results_history():
    """Load accumulated results: list of completed match records."""
...
def save_results_history(results):
    save_json(RESULTS_FILE, {'results': results, 'updated': time.time()})
...
def check_past_predictions():
    ...
    # Prune results history older than 3 days (keep enough for yesterday lookups)
    ...
    save_results_history(results_history)
...
def harvest_completed_events():
    ...
    # Use commonThumbnailEvents — it may include recent completed
    # Also try scanning gameIds slightly before current upcoming ones
    try:
        r = requests.get('https://www.sportybet.com/api/ng/factsCenter/commonThumbnailEvents', params={
            'sportId': SPORT, 'productId': 3,
        }, headers=HEADERS, timeout=15)
...
def find_slot(history, target_est):
    ...
    # Filter to yesterday's results only
    yesterday_results = []
    for r in history:
    ...
    # Find closest slot to target time-of-day
...
def energy(slot, home, away):
    ...
    sf = None  # Same fixture
    hd = {'s': 0, 'c': 0, 't': 0}  # Home team data
    ad = {'s': 0, 'c': 0, 't': 0}  # Away team data
    ...
    for m in slot:
        ...
        # Home team appeared as home yesterday
        if h == home:
            hd = {'s': hs, 'c': aws, 't': hs + aws}
        # Home team appeared as away yesterday
        elif a == home:
            hd = {'s': aws, 'c': hs, 't': hs + aws}
        
        # Away team appeared as home yesterday
        if h == away:
            ad = {'s': hs, 'c': aws, 't': hs + aws}
        # Away team appeared as away yesterday
        elif a == away:
...
def sec_b(en):
    ...
    # ── SKIP RULES (optimized: A+F removed, kept C+D+E) ──
    # C: Compression — both teams had >=4 total in slot
    if h['t'] >= 4 and a['t'] >= 4:
...
def analyze(event, blacklist, results_history):
    ...
    # Section A: Pre-match JSON
    a = sec_a(event)
    
    # Section B: Yesterday same-slot energy
    ...
    if has_section_b:
        ...
        # COMBINED score: A + B
        combined = a['sc'] + b['sc']
        cmax = a['mx'] + 14  # A max + B max (14)
        cpct = round(combined / cmax * 100) if cmax > 0 else 0
        
        # If Section B says SKIP, respect it
        if b['skip']:
            verdict = 'SKIP'
        elif cpct >= 70:
            verdict = 'LOCK'
        ...
        # No Section B data available — use Section A only (degraded mode)
        b = {'skip': False, 'reason': 'NO_DATA', 'sc': 0, 'conf': 'N/A', 'reasons': ['No yesterday data']}
        combined = a['sc']
        cmax = a['mx']
        cpct = a['pct']
        
        # Section A-only thresholds (stricter since no B confirmation)
        if cpct >= 75:
            verdict = 'LOCK'
        elif cpct >= 55:
            verdict = 'PICK'
...
def run():
    ...
    print("=" * 60)
    print("VFL ENGINE v3.6 — ONIMIX TECH")
    print("Section A + Section B + Failure Monitor")
    print(scan_time)
    print("=" * 60)
    ...
    lines = [
        "<b>VFL MEGA — ONIMIX ELITE v3.6</b>",
        scan_time,
    ...
    lines.extend([
        "",
        "<i>Engine v3.6 | A+B Active | Monitor On | ONIMIX TECH</i>",
    ])
